...
void Renderer::RootSignature::Initialize(int srvCount, int uavCount, int cbCount, D3D12_STATIC_SAMPLER_DESC& sampler)
{
	...
	m_rootSignatureDesc.Init_1_1(cbCount + 1, paramenters, 1, &m_sampler, rootSignatureFlags);
}

void Renderer::RootSignature::Initialize(int srvCount, int cbCount, D3D12_STATIC_SAMPLER_DESC& sampler)
{
	m_sampler = sampler;
...
void Renderer::RootSignature::Finalize(Microsoft::WRL::ComPtr<ID3D12Device>& device)
{
	...
	CD3DX12_ROOT_PARAMETER1 paramenters[3];
	paramenters[0].InitAsDescriptorTable(1, &rangeTable, D3D12_SHADER_VISIBILITY_PIXEL);
	paramenters[1].InitAsConstantBufferView(0);
	paramenters[2].InitAsConstantBufferView(1);

	D3D12_ROOT_SIGNATURE_FLAGS rootSignatureFlags =
		D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT;

	m_rootSignatureDesc.Init_1_1(3, paramenters, 1, &m_sampler, rootSignatureFlags);
