...
void Volume::CopyDescriptors(Microsoft::WRL::ComPtr<ID3D12Device5>& device)
{
	...
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mDivergence.GetNSVUavCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 2 divergence UAV
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mPressure.GetNSVUavCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 3 pressure UAV
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mPressureTemp.GetNSVUavCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 4 pressureTemp UAV
	handle.Offset(1, offset);

	device->CopyDescriptorsSimple(1, handle, mDensity.GetNSVSrvCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);  // 5
	handle.Offset(1, offset);
	...
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mDensityTemp.GetNSVSrvCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 10
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mVelocityTemp.GetNSVSrvCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mDensityTemp.GetNSVUavCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 12 densityTemp UAV
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mVelocityTemp.GetNSVUavCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 13 velocityTemp UAV
	handle.Offset(1, offset);
	device->CopyDescriptorsSimple(1, handle, mVorticity.GetNSVUavCPUHandle(), D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV); // 14 vorticity UAV
	handle.Offset(1, offset);
