...
namespace Renderer {
    ...
    void ModelLoader::LoadPbr32(std::string filename, bool loadAnimation)
    {
        ...
        Matrix tr;
        ProcessPbrNode32(pScene->mRootNode, pScene, tr);
    ...
    void ModelLoader::ProcessPbrNode32(aiNode* node, const aiScene* scene, DirectX::SimpleMath::Matrix tr)
    {
        Matrix m;
        ai_real* temp = &node->mTransformation.a1;
        float* mTemp = &m._11;
        ...
        for (UINT i = 0; i < node->mNumMeshes; i++) {
            ...
            aiMesh* mesh = scene->mMeshes[node->mMeshes[i]];
            m_animeData.meshNameToId[mesh->mName.C_Str()] = meshCount++;
            auto newMesh = this->ProcessPbrMesh32(mesh, scene);
            newMesh.m_name = mesh->mName.C_Str();
            ...
            pbrMeshes32.push_back(newMesh);
        }

        for (UINT i = 0; i < node->mNumChildren; i++) {
            this->ProcessPbrNode32(node->mChildren[i], scene, m);
        }
    }
    ...
    PbrMeshData32 ModelLoader::ProcessPbrMesh32(aiMesh* mesh, const aiScene* scene)
    {
        {
            // Data to fill
            std::vector<PbrVertex> vertices;
